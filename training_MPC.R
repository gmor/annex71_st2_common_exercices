...
one_day_prediction <- function(n_steps_prediction = 1){
  
  df_aux <- df
  n_steps_prediction = 1
  for (i in 1:23) {

    predv <- prediction_scenario_different_steps(
      mod_q = mod_q, 
      mod_ti = mod_ti,
      mod_tsupply = mod_tsupply,
      df = df,
...
calculate_many_errors <- function(val_dates, n_steps_prediction){
