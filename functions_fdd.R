...
tune_model_input <- function(df,params){
  ...
  # HP consumption
  for (l in 0:max(params[c("mod_tsupply_lags_hp_cons","mod_hp_cons_ar","mod_tsupply_ar")])){
    df[,paste0("hp_cons_l",l)] <- dplyr::lag(df[,"hp_cons"],l)
    df[,paste0("hp_status_l",l)] <- dplyr::lag(df[,"hp_status"],l)
  }
  
  #Additional heaters
  for (l in 0:params["mod_tsupply_lags_hp_heaters"]){
    df[,paste0("hp_heaters_l",l)] <- dplyr::lag(df[,"hp_heaters"],l)
  }
...
calculate_model_q <- function(params, df, train_dates, output="aic"){
  ...
  formula <- as.formula(sprintf("hp_cons_l0 ~ 
                                1 + hp_cons_l1 + %s + %s + %s +%s",
                                paste0("tsupply_l",0:params["mod_hp_cons_lags_tsupply"],collapse=" + "),
                                ...
                                paste0("hp_heaters_l",0:params["mod_tsupply_lags_hp_heaters"],"",collapse=" + ")
...
calculate_model_tsupply <- function(params, df, train_dates, output="aic"){
  ...
  formula <- as.formula(sprintf("tsupply_l0 ~ 
      0 + %s + %s + %s",
                                ...
                                paste0(mapply(function(x){sprintf("hp_heaters_l%s",x,x)},0:params["mod_tsupply_lags_hp_heaters"]),collapse=" + ")
                                # paste0(mapply(function(x){sprintf("hp_heaters_l%s*hp_cons_l%s",x,x)},0:params["mod_tsupply_lags_hp_heaters"]),collapse=" + ")
...
prediction_scenario <- function(mod_q, mod_ti, mod_tsupply, df, rows_to_filter=NULL, hp_tset_24h, params, ts_prediction=NULL){
  hp_tset_24h <- as.numeric(hp_tset_24h)
  df <- tune_model_input(df,params)
  ...
  df <- lapply(ts_prediction,function(ts_){
    ...
    for (i in 1:nrow(df_)){
      ...
      for (l in 1:max(params[c("mod_hp_cons_ar","mod_tsupply_lags_hp_cons","mod_tsupply_ar")])){
...
optimizer_model_parameters <- function(X, class_per_feature, nclasses_per_feature, min_per_feature, 
  ...
  predv <- prediction_scenario(
    ...
    hp_tset_24h = ifelse(df$hp_status==0,"NA",as.character(ceiling(df$hp_tset))),
    params = params,
    ts_prediction = NULL
  )
  ...
  a <- acf(mod_tsupply$model$tsupply_l0-mod_tsupply$fitted.values,plot = F)
