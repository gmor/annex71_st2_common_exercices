...
horizon <- 24 #set horizon for training
...
## MPC model parameters optimization, apart from the optimization it was tuned to improve the performance
## the model parameters are loaded after the commented GA
#
...
#Its created a similar dataset as the previous one but to reflect the free floating behaviour from the temp before the 
#setback untill it reach the setback low temp boundary
...
#Figure for paper - case of curve crossing explenation
l_size = 1.2
ggplot() +
...
#Figure for paper, to explain the evolution of the horizon in the papaer is placed a case example created below
setback <- 20:1 #remining setback time
horizon_optim <- 5
hours_heating <- 10
hours_heating_from_cooling <- 12
min_horizon <- {}
#Compute the horizon following the determined methodology
...
#Initialization of the vectors in which are stored the predicted values (_df) and the real values (_l0_df)
ti_df <- {}
ti_l0_df <- {}
tsupply_df <- {}
tsupply_l0_df <- {}
...
hp_cons_df <- {}
hp_cons_l0_df <- {}
ii_df <- {}#vector of horizon length used to identify the horizon in each prediction
...
#Compute R2 and CVRMSE of all the forecasted values
...
#initialization of the vectors to compute the R2, MAPE and CVRMSE of each horizon forecast
cop_df_MAPE <- cop_df
...
use_condaenv()#To use Python environment
py<-import("os")
py<-import("fmpy")
py<-import("sys")
...
while (time <= max_time) { #<=
  ...
  if (time%%3600 == 0) {
    ...
    # system.time( #to conut the neede time ro tun the MPC
    #Compute GA
    ...
    print("Optimized tsupply:")#show the optimal solution on screen
    ...
    #Compute the GA solution to extract the forecasted results
...
#cut the df to only analysis the the specific period 
...
#count the number of iterations in discomfort
time_dis <- 0
...
#Figure for paper - Grafica Tin + boundaries
l_size = 1.2
...
#Figure for paper - Grafica Tin + boundaries + energy cost
ggplot(df_eval[df_eval$time_to_predict_step >= as.POSIXct(x = "2019-01-04 00:00:00 UTC", tz = "UTC") &
                 df_eval$time_to_predict_step <= as.POSIXct(x = "2019-01-07 00:00:00 UTC", tz = "UTC"),]) +
...
#Add min/max boundaries in the df_result_20s dataset
